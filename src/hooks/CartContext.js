...
export const CartProvider = ({ children }) => {
	...
	const updateLocalStorage = async (products) => {
		await localStorage.setItem('uppedidos:cartInfo', JSON.stringify(products))
	}
	...
	useEffect(() => {
		const loadUserData = async () => {
			const clientCartData = await localStorage.getItem('uppedidos:cartInfo')
...
export const useCart = () => {
	...
	if(!context){
		throw new Error('Falha ao carregar o context: CartContext')
	}
	return context
}
